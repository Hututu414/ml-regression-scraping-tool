...
def _download_yfinance(tickers, start_date, end_date, freq):
    try:
...
def _load_ohlcv_csv(uploaded_file):
...
class CNN1D:
    ...
    def state_dict(self):
        sd = {"conv1": self.conv1.state_dict(), "fc": self.fc.state_dict()}
        return sd
...
def run_dl_lab(params, uploaded_file, dirs):
    ...
    try:
        if params.get("data_mode") == "CSV 上传":
            ...
            df_all = _download_yfinance(params.get("tickers") or [], params.get("start_date"), params.get("end_date"), params.get("freq"))
...
def export_dl_lab(result, dirs, tag):
    ...
    report_path_csv = Path(dirs["reports"]) / ("dl_report_" + suffix + ".csv")
    report_path_json = Path(dirs["reports"]) / ("dl_report_" + suffix + ".json")
    ...
    pred_path = Path(dirs["reports"]) / ("dl_pred_" + suffix + ".json")
