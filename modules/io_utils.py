...
def ensure_output_dirs(root):
    ...
    for p in [outputs, figures, data_dir, models, reports, cache_dir]:
        ...
        for c in cols:
            ...
            for cand in candidates:
                if cand in name:
                    return c
        return None
    date_col = find({"date", "datetime", "time", "timestamp", "交易日期", "日期"})
...
def cache_path(dirs, prefix, payload):
    h = stable_hash(payload)
    name = safe_filename(prefix) + "_" + h + ".csv"
