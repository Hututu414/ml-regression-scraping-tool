...
def _yf_interval(freq):
    return "1wk" if str(freq) == "周" else "1d"
...
def _prepare_price_df_from_csv(uploaded_file, freq):
...
def _prepare_price_df_yfinance(ticker, start_date, end_date, freq):
    try:
...
def run_stock_analytics(params, asset_csv_file, market_csv_file, dirs):
    try:
        ...
        if params.get("data_mode") == "CSV 上传":
            ...
            if market_csv_file is not None:
                ...
                try:
                    market_df = _prepare_price_df_yfinance(params.get("benchmark_ticker"), params.get("start_date"), params.get("end_date"), freq=freq)
                except Exception as e:
                    market_df = None
        else:
            asset_df = _prepare_price_df_yfinance(params.get("ticker"), params.get("start_date"), params.get("end_date"), freq=freq)
            market_df = _prepare_price_df_yfinance(params.get("benchmark_ticker"), params.get("start_date"), params.get("end_date"), freq=freq)

        asset_df = _filter_by_date(asset_df, params.get("start_date"), params.get("end_date"))
        market_df = _filter_by_date(market_df, params.get("start_date"), params.get("end_date")) if market_df is not None else None

        asset_cols = io_utils.detect_ohlcv_columns(asset_df.reset_index())
        market_cols = io_utils.detect_ohlcv_columns(market_df.reset_index()) if market_df is not None else {}
        if str(price_field) == "Adj Close":
            asset_price_col = asset_cols.get("adj_close") or asset_cols.get("close")
            market_price_col = market_cols.get("adj_close") or market_cols.get("close")
        else:
            asset_price_col = asset_cols.get("close") or asset_cols.get("adj_close")
            market_price_col = market_cols.get("close") or market_cols.get("adj_close")
        if asset_price_col is None:
            raise ValueError("标的价格列缺失")
        if market_df is None:
            raise ValueError("市场基准缺失，请上传市场基准 CSV 或启用网络")
...
def export_stock_analytics(result, dirs, tag, export_format):
    ...
    data_df = result.get("data") or pd.DataFrame()
    ...
    if str(export_format) == "Excel":
        path = Path(dirs["reports"]) / (base + ".xlsx")
        io_utils.save_excel(
            {
                "data": data_df,
                "regression": reg,
        ...
        out.append(io_utils.save_df_csv(data_df, Path(dirs["reports"]) / (base + "_data.csv")))
        out.append(io_utils.save_df_csv(pd.DataFrame([reg]), Path(dirs["reports"]) / (base + "_regression.csv")))
        out.append(io_utils.save_df_csv(pd.DataFrame([diff_stats]), Path(dirs["reports"]) / (base + "_diff_stats.csv")))
        out.append(io_utils.save_df_csv(pd.DataFrame([params]), Path(dirs["reports"]) / (base + "_params.csv")))
    return out
